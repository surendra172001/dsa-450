...
public:
    int findPivot(vector<int> &nums)
    {
        int n = nums.size();
        int s = 0, e = n - 1;
        int pivotIdx = -1; // rightmost element such that a[s] < a[idx] and (idx < e and a[idx] > a[e])

        bool comingFromRight = true;
        // I am initializing as true since we can assume that e is the immediate left of mid index of a larger array which was having larger sorted right subarray and we came left since pivotIdx was residing left of that right subarray
    ...
    int search(vector<int> &nums, int target)
    {
        ...
        assert((pivotIdx != -1) && "Wrong assumptions");

        // cout << idx << endl;

        int targetIdx1 = (lower_bound(nums.begin(), nums.begin() + pivotIdx + 1, target) - nums.begin());

        if ((targetIdx1 != pivotIdx + 1) && (nums[targetIdx1] == target))
        {
            return targetIdx1;
        }

        int targetIdx2 = (lower_bound(nums.begin() + pivotIdx + 1, nums.end(), target) - nums.begin());
